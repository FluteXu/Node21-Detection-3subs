...
class GaussianBlurTransform(Transform):
    def __init__(self, sigma_range, kernel_size):
        """
        Args:
            sigma_range: std for guassian kernel
            kernel_size: conv. filter
        """
        # TODO decide on PIL vs opencv
        super().__init__()
        self.gBlur = T2.GaussianBlur(kernel_size=kernel_size, sigma=sigma_range)
        self._set_attributes(locals())

    def apply_image(self, img):
        img = np.squeeze(img, -1).astype(np.float64)
        img = np.expand_dims(img, 0).astype(np.float64)
        img = torch.tensor(img, dtype=torch.float64)
        img = self.gBlur(img)
        img = np.asarray(img).astype(np.float64)
        img = np.squeeze(img, 0).astype(np.float64)
        img = np.expand_dims(img, -1).astype(np.float64)
        return img
